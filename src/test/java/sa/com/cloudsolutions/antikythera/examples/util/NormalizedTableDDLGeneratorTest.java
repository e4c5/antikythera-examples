...
class NormalizedTableDDLGeneratorTest {
    ...
    @Test
    void testLiquibaseModeNonNumericIdNoAutoIncrement() {
        EntityProfile uuidProfile = new EntityProfile(
                "Customer",
                "old_customer",
                List.of(
                        new FieldProfile("id", "id", true, false, "String", true)
                ),
                List.of()
        );
        ViewDescriptor view = new ViewDescriptor(
                "old_customer", "customer", List.of("customer"),
                List.of(new ColumnMapping("id", "customer", "id")),
                List.of()
        );

        List<String> result = generator.generate(view, uuidProfile, NormalizedTableDDLGenerator.MODE_LIQUIBASE);
        assertFalse(result.get(0).contains("autoIncrement=\"true\""), "String ID should not have autoIncrement");
    }

    @Test
    void testLiquibaseModeNonAutoGeneratedIdNoAutoIncrement() {
        EntityProfile manualIdProfile = new EntityProfile(
                "Customer",
                "old_customer",
                List.of(
                        new FieldProfile("id", "id", true, false, "Long", false)
                ),
                List.of()
        );
        ViewDescriptor view = new ViewDescriptor(
        ...
        List<String> result = generator.generate(view, manualIdProfile, NormalizedTableDDLGenerator.MODE_LIQUIBASE);
        assertFalse(result.get(0).contains("autoIncrement=\"true\""), "Manual Long ID should not have autoIncrement");
    }
