...
public class NormalizedTableDDLGenerator {
    ...
    public List<String> generate(ViewDescriptor view, EntityProfile sourceProfile, String ddlMode) {
        ...
        Map<String, Boolean> columnIsAutoGenerated = new HashMap<>();
        if (sourceProfile != null) {
            for (var field : sourceProfile.fields()) {
                ...
                columnIsAutoGenerated.put(field.columnName(), field.isAutoGenerated());
            }
        }
        ...
        for (String table : order) {
            ...
            if (MODE_RAW_SQL.equals(ddlMode)) {
                result.add(buildRawSql(table, cols, columnJavaType, columnNullable, columnIsId, columnIsAutoGenerated));
            } else {
                result.add(buildLiquibaseChangeset(table, cols, columnJavaType, columnNullable, columnIsId, columnIsAutoGenerated));
            }
        }
    ...
    private String buildLiquibaseChangeset(String table, List<ColumnMapping> cols,
        ...
        for (ColumnMapping cm : cols) {
            content.append("|").append(cm.sourceColumn())
                   .append("|").append(cm.viewColumn());
        }
        String idSuffix = Integer.toHexString(content.toString().hashCode());

        StringBuilder sb = new StringBuilder();
        sb.append("<changeSet id=\"create_table_").append(table).append("_")
          .append(idSuffix).append("\" author=\"antikythera\">\n");
        sb.append("    <createTable tableName=\"").append(table).append("\">\n");

        for (ColumnMapping cm : cols) {
            String javaType = typeMap.get(cm.viewColumn());
            String lbType = toLiquibaseType(javaType);
            boolean isId  = Boolean.TRUE.equals(isIdMap.get(cm.viewColumn()));
            boolean isAutoGenerated = Boolean.TRUE.equals(autoGenMap.get(cm.viewColumn()));
            boolean nullable = !Boolean.FALSE.equals(nullableMap.getOrDefault(cm.viewColumn(), true));

            sb.append("        <column name=\"").append(cm.sourceColumn())
              .append("\" type=\"").append(lbType).append("\"");
            if (isId && isAutoGenerated && isNumericType(javaType)) {
    ...
    /** Returns true if the Java type is numeric (Long, Integer, Short, or their primitives). */
    public static boolean isNumericType(String javaType) {
        if (javaType == null) return false;
        return javaType.equals("Long") || javaType.equals("long") ||
               javaType.equals("Integer") || javaType.equals("int") ||
