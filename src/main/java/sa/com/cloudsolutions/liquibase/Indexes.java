...
public class Indexes {
    ...
    private static class LiquibaseIndexHandler extends LiquibaseParser {
        ...
        private void handleCreateIndex(Attributes atts) {
            inCreateIndex = true;
            createIndexName = firstNonEmpty(atts.getValue("indexName"), atts.getValue("name"));
            createIndexTable = firstNonEmpty(atts.getValue(TABLE_NAME_ATTR), atts.getValue(TABLE));
        ...
        private void handleColumn(Attributes atts) {
            if (inCreateIndex) {
        ...
        private void handleConstraints(Attributes atts) {
        ...
        private void processCreateIndexSql(String sql) {
            ...
            String cleaned = cleanSql(normalized);

            Matcher m = createIndexPattern().matcher(cleaned);
            if (!m.find()) {
                try {
            ...
            extractAndAddIndex(m);
        }

        private void extractAndAddIndex(Matcher m) {
            ...
            List<String> cols = parseColumns(colsCsv);
        ...
        private void processDropSql(String sql) {
            if (isBlank(sql)) return;
            String normalized = normalizeSql(sql);
            String upper = normalized.toUpperCase();

            if (upper.startsWith("DROP") && upper.contains(INDEX)) {
                processDropIndexSql(normalized);
                return;
            }

            if (upper.startsWith("ALTER TABLE")) {
                processAlterTableSql(normalized);
            }
        }

        private void processDropIndexSql(String normalized) {
            String cleaned = cleanSql(normalized);
        ...
        private void processAlterTableSql(String normalized) {
        ...
        private String cleanSql(String normalized) {
            return normalized
        ...
        private Pattern createIndexPattern() {
            return Pattern.compile("(?i)^CREATE\\s+(UNIQUE\\s+)?INDEX\\s+([\"`\\w.]+)\\s+ON\\s+([\"`\\w.]+)\\s*\\((.*?)\\)");
        }

        private List<String> parseColumns(String colsCsv) {
            return Arrays.stream(colsCsv.split(","))
                .map(String::trim)
                .filter(s -> !s.isEmpty())
                .map(c -> {
        ...
        private void removeIndexByNameAnyTable(Map<String, Set<IndexInfo>> map, String name) {
        ...
        private void add(Map<String, Set<IndexInfo>> map, String table, IndexInfo index) {
            map.computeIfAbsent(table, k -> new HashSet<>()).add(index);
        }

        private void removeIndexByName(Map<String, Set<IndexInfo>> map, String table, String name) {
        ...
        private void removeIndexByColumnsOrdered(Map<String, Set<IndexInfo>> map, String table, List<String> cols) {
        ...
        private void removeUniqueConstraint(Map<String, Set<IndexInfo>> map, String table, String name, List<String> cols) {
            Set<IndexInfo> list = map.get(table);
            if (list == null) return;
            if (!isBlank(name)) {
                String target = name.toLowerCase();
        ...
        private void removePrimaryKey(Map<String, Set<IndexInfo>> map, String table, String name) {
            Set<IndexInfo> list = map.get(table);
            if (list == null) return;
            if (!isBlank(name)) {
                String target = name.toLowerCase();
        ...
        private boolean orderedColumnsMatch(List<String> cols, List<String> targetLowerOrdered) {
